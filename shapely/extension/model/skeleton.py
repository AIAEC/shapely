"""
Reference: https://github.com/Botffy/polyskel
...
class _LAVertex:
    def __init__(self, point, edge_left, edge_right, direction_vectors=None):
        ...
        self._bisector = Ray2(self.point, euclid_operator.add(*creator_vectors) * (-1 if self.is_reflex else 1))
...
def _skeletonize(exterior, interiors=None):
    """
...
def skeletonize(polygon: Polygon) -> List[LineString]:
    if not polygon.is_valid or polygon.is_empty:
        raise ValueError('expect valid, non-empty polygon, given invalid or empty polygon')
    if not polygon.is_simple:
        raise ValueError('expect simple polygon, given non-simple polygon')
    ...
    subtrees = _skeletonize(exterior=exterior, interiors=interiors)
    lines: List[LineString] = []
    for subtree in subtrees:
        for sink in subtree.sinks:
            if subtree.source != sink:
                lines.append(LineString([subtree.source, sink]))
...
class Skeleton:
    ...
    def trunk_segments(self) -> List[LineString]:
        if isinstance(self._geom, Polygon):
            boundary = unary_union(self._geom.ext.decompose(MultiLineString).to_list())
            return lfilter(lambda seg: not seg.intersects(boundary), self._skeleton_lines)

        return self.full_segments()

    def branch_segments(self) -> List[LineString]:
        if isinstance(self._geom, Polygon):
            boundary = unary_union(self._geom.ext.decompose(MultiLineString).to_list())
            return lfilter(lambda seg: seg.intersects(boundary), self._skeleton_lines)

        return []

    def full_segments(self) -> List[LineString]:
        return self._skeleton_lines

    def trunks(self) -> List[LineString]:
        return linemerge(self.trunk_segments()).ext.flatten(LineString).to_list()
