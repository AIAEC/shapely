...
class TestAngleAttachOffset:
    def test_offset_exterior_edge_inner_hit_hit_default_handler(
        ...
        edge_seqs = Offset(edge, offset_handler_cls=AngleAttachOffsetHandler).offset(10)
    ...
    def test_offset_exterior_edge_seq_inner_hit_hit_default_handler_with_reverse_closure(
        ...
        edge_seqs = Offset(edge_seq, offset_handler_cls=AngleAttachOffsetHandler).offset(10)
        assert all(isinstance(edge_seq, EdgeSeq) for edge_seq in edge_seqs)
        assert len(edge_seqs) == 1
    ...
    def test_offset_exterior_edge_inner_connected_in_in_default_handler(
        ...
        assert stretch.closures[0].shape.equals(
            loads(
                'POLYGON ((0 0, 10 0, 10 2, 8 2, 8 18, 22 18, 22 2, 19.999999999999993 2, 20 0, 30 0, 30 20, 0 20, 0 0))'))
    ...
    def test_offset_exterior_edge_inner_out_out_default_handler(self,
        ...
        edge_seqs = Offset(edge, offset_handler_cls=AngleAttachOffsetHandler).offset(30)
        assert len(edge_seqs) == 0
    ...
    def test_offset_interior_edge_hit_hit_contained_hole_default_handler(
        ...
        edge_seqs = Offset(edge, offset_handler_cls=AngleAttachOffsetHandler).offset(18)
        assert all(isinstance(edge_seq, EdgeSeq) for edge_seq in edge_seqs)
        assert len(edge_seqs) == 1
        edge_seq = edge_seqs[0]
        assert isinstance(edge_seq, EdgeSeq)
    ...
    def test_offset_interior_edge_hit_in_default_handler(
        ...
        edge_seqs = Offset(edge, offset_handler_cls=AngleAttachOffsetHandler).offset(5)
        assert all(isinstance(edge_seq, EdgeSeq) for edge_seq in edge_seqs)
        assert len(edge_seqs) == 1
        edge_seq = edge_seqs[0]
        assert isinstance(edge_seq, EdgeSeq)
        ...
        assert edge_seq.shape.equals(loads("LINESTRING (18 15, 10.000000000000028 15)"))
        assert stretch.closure('0') is None, 'origin closure should be removed'
        assert len(stretch.pivots) == 16
        assert len(stretch.edges) == 16
        ...
        assert len(stretch.closures) == 1
        assert len(stretch.closures[0].interiors) == 1
        assert stretch.closures[0].shape.almost_equals(
            loads(
    ...
    def test_offset_interior_outer_into_hole_default_handler(self, stretch_interior_offset_hit_hit):
        ...
        edge_seqs = Offset(edge, offset_handler_cls=AngleAttachOffsetHandler).offset(-2)
        assert all(isinstance(edge_seq, EdgeSeq) for edge_seq in edge_seqs)
        assert len(edge_seqs) == 1
        edge_seq = edge_seqs[0]
        assert isinstance(edge_seq, EdgeSeq)
    ...
    def test_offset_interior_outer_into_inner_space_default_handler(self, stretch_interior_offset_hit_hit):
        ...
        edge_seqs = Offset(edge, offset_handler_cls=AngleAttachOffsetHandler).offset(-6)
        assert len(edge_seqs) == 0
        assert stretch.closure('0') is None, 'origin closure should be removed'
    ...
    def test_offset_interior_outer_into_outer_space_default_handler(self, stretch_interior_offset_hit_hit):
        ...
        edge_seqs = Offset(edge, offset_handler_cls=AngleAttachOffsetHandler).offset(-15)
        assert len(edge_seqs) == 1
        edge_seq = edge_seqs[0]
        assert isinstance(edge_seq, EdgeSeq)
        assert len(edge_seq) == 1
    ...
    def test_offset_interior_outer_into_hole_hit_in_default_handler(self, stretch_interior_offset_into_hole_in_hit):
        ...
        edge_seqs = Offset(edge, offset_handler_cls=AngleAttachOffsetHandler).offset(-13)
        assert len(edge_seqs) == 1
        edge_seq = edge_seqs[0]
        assert isinstance(edge_seq, EdgeSeq)
        assert len(edge_seq) == 1
    ...
    def test_offset_exterior_edge_outside_of_closure_naive_handler(
        ...
        assert closures[0].shape.almost_equals(loads(
            'POLYGON ((0 0, 10 0, 10 20, 0 20, 0 0))'))

        assert closures[1].shape.almost_equals(
            loads('POLYGON ((20 20, 20 12, 22 12, 22 8, 20 8, 20 0, 30 0, 30 20, 20 20))'))
    ...
    def test_offset_exterior_edge_inner_hit_hit_with_strict_attach_handler(
        ...
        assert edge_seq.shape.almost_equals(LineString([(20, 5), (10, 5)]))
        assert len(stretch.closures) == 1
        assert len(stretch.pivots) == 8
        assert len(stretch.edges) == 8
        assert stretch.closures[0].shape.almost_equals(
    ...
    def test_offset_exterior_edge_inner_switching_to_perpendicular_attachment_with_strict_attach_handler(
        ...
        assert isinstance(edge_seq, EdgeSeq)
        assert len(edge_seq) == 1
        assert edge_seq.shape.almost_equals(LineString([(17, 5), (13, 5)]))
    ...
    def test_offset_exterior_edge_inner_extrude_with_strict_attach_handler(
        ...
        assert closures[0].shape.almost_equals(loads(
            'POLYGON ((0 0, 13.000000000000014 0, 13 10, 0 10, 0 0))'))

        assert closures[1].shape.almost_equals(
            loads('POLYGON ((30 0, 30 10, 17 10, 17.000000000000014 0, 30 0))'))
    ...
    def test_offset_interior_edge_inner_switching_to_perpendicular_attachment_with_strict_offset_handler(
        ...
        edge_seqs = Offset(edge, offset_handler_cls=AngleAttachOffsetHandler).offset(7)
        assert len(edge_seqs) == 1
        edge_seq = edge_seqs[0]
        assert isinstance(edge_seq, EdgeSeq)
        assert len(edge_seq) == 1
        assert edge_seq.shape.almost_equals(LineString([(13, 27), (17, 27)]))
    ...
    def test_offset_inner_box_exterior_edge_towards_outer_space(self, stretch_outer_and_inner_closures):
        ...
        edge_seqs = Offset(edge, offset_handler_cls=AngleAttachOffsetHandler).offset(-0.1)
        assert len(edge_seqs) == 1
        edge_seq = edge_seqs[0]
        assert isinstance(edge_seq, EdgeSeq)
        assert len(edge_seq) == 1

        assert edge_seq.shape.almost_equals(LineString([(0.6, 0.4), (0.5, 0.4)]))
        closures = sorted(stretch.closures, key=lambda closure: closure.shape.centroid.x)
    ...
    def test_offset_inner_box_exterior_edge_towards_inner_space(self, stretch_outer_and_inner_closures):
        ...
        edge_seqs = Offset(edge, offset_handler_cls=AngleAttachOffsetHandler).offset(0.05)
        assert len(edge_seqs) == 1
        edge_seq = edge_seqs[0]
        assert isinstance(edge_seq, EdgeSeq)
        assert len(edge_seq) == 1

        assert edge_seq.shape.almost_equals(LineString([(0.5, 0.55), (0.6, 0.55)]))
        closures = sorted(stretch.closures, key=lambda closure: closure.shape.centroid.x)
        assert len(closures) == 2
        assert len(closures[0].interiors) == 1
        assert closures[0].shape.almost_equals(
    ...
    def test_offset_exterior_inner_and_union_space_to_multiple_reverse_closures(
        ...
        with pytest.raises(AssertionError):
            Offset(edge_seq, offset_handler_cls=AngleAttachOffsetHandler).offset(1)
    ...
    def test_offset_exterior_edges_one_by_one_to_inner_space(
        ...
        for edge in edge_seq:
            Offset(edge, offset_handler_cls=AngleAttachOffsetHandler).offset(1)
    ...
    def test_cargo_when_offset_exterior_edge_inner(self, stretch_2_boxes):
        ...
        edges_seqs = Offset(stretch.edge('(1,2)'), offset_handler_cls=AngleAttachOffsetHandler).offset(0.1)
    ...
    def test_cargo_when_offset_exterior_edge_seq_inner(self, stretch_exterior_offset_hit_hit_with_reverse_closure):
        ...
        edge_seqs = Offset(edge_seq, offset_handler_cls=AngleAttachOffsetHandler).offset(1)
    ...
    def test_offset_twice(self, stretch_of_three_closures):
       ...
       3├──────────────┬──────┤5
        ...
        edge_seq = EdgeSeq([stretch.edge('(3,4)'),
                            stretch.edge('(4,5)')])
        for edge in edge_seq:
            Offset(edge, offset_handler_cls=AngleAttachOffsetHandler).offset(150)
    ...
    def test_angle_attach_offset_twice(self,
    ...
    def test_angle_attach_offset_twice_reverse_order(self,
                                                     stretch_of_three_closures):
        """
    ┌─────────────────────┐
    │                     │
    ...
    def test_angle_attach_offset_to_eliminate_region(self,
                                                     stretch_of_three_closures):
        """
    ┌─────────────────────┐
    │                     │
    ...
    └──────────────┴──────┘
        ...
        for edge in [stretch.edge('(4,5)'),
                     stretch.edge('(3,4)')]:
            Offset(edge, offset_handler_cls=AngleAttachOffsetHandler).offset(190)
        closures = stretch.closures
        closures.sort(key=lambda closure: closure.shape.centroid.x)

        assert len(closures) == 2
        assert closures[0].shape.equals(loads('POLYGON ((0 0, 75 0, 75 10, 75 200, 0 200, 0 10, 0 0))'))
        assert closures[1].shape.equals(loads('POLYGON ((75 0, 100 0, 100 10, 100 200, 75 200, 75 10, 75 0))'))
